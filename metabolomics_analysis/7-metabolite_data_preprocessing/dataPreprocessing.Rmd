...
metaDataMBX <- metaDataMBX %>% dplyr::select(External.ID,Participant.ID,diagnosis,Yogurt.or.other.foods.containing.active.bacterial.cultures..kefir..sauerkraut.)
#rename columns of metaDataMBX
colnames(metaDataMBX) <- c("ExternalID","ParticipantID","disease","Yogurt consumption")

#filter out patients with UC (only want to compare non-IBD vs CD patients)
metaDataMBX <- subset(metaDataMBX, metaDataMBX$disease == "CD" | metaDataMBX$disease == "nonIBD")

#delete patients for which no data on yoghurt consumption is available  
metaDataMBX<- metaDataMBX[!(metaDataMBX$Yogurt.or.other.foods.containing.active.bacterial.cultures..kefir..sauerkraut.=="") , ]
...
#write only CD versus nonIBD comparison
mbxDataCDnonIBD <- mbxData[ ,(mbxData[1, ] == "CD" | mbxData[1, ] == "nonIBD")]
#add hmdb ID again
mbxDataCDnonIBD <- cbind(mbxData[,1:2],mbxDataCDnonIBD)
colnames(mbxDataCDnonIBD)[1]="HMBDB.ID"
colnames(mbxDataCDnonIBD)[2] <- "Compound.Name"
write.table(mbxDataCDnonIBD, "output/mbxDataUC_nonIBD.csv", sep =",", row.names = FALSE)
#confused about what row 128 does


#DELETING THIS STEP BECAUSE NOT RELEVANT FOR NOW 
